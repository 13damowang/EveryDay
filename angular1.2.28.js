 ...
 * 该对象提供了用于生成丰富错误消息的实用程序，Angular.他可以被称为如下：
 *
 ...
 * take.
 * 上述例子中的命名空间创建一个最小误差法实例。这个
 * 产生的误差有一个命名空间的example.one错误代码。这个
 * 产生的错误将用{值的值替换{ 0 }，并用{ 1 }替换
 * 酒吧价值。对象不受其参数数的限制。
 * 带。
 *
 * If fewer arguments are specified than necessary for interpolation, the extra
 * interpolation markers will be preserved in the final string.
 * 如果指定的参数小于插值所需的参数，则额外的插值标记将保存在最终字符串中
 *
    ...
    var code = arguments[0],
      ...
      stringify = function (obj) {
        if (typeof obj === 'function') {
          return obj.toString().replace(/ \{[\s\S]*$/, ''); //去除字符串的空白符
        } else if (typeof obj === 'undefined') {
          return 'undefined';
        } else if (typeof obj !== 'string') {
          return JSON.stringify(obj); //如果不是字符串类型，就转成字符串
        }
        return obj;
      },
      message, i;
